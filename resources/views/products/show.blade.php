...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{
    ...
    <div class="grid lg:grid-cols-2 gap-10 xl:gap-16">
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-wrap gap-2">
                @if($product->is_new)
                    <span class="inline-block bg-violet-100 text-violet-700 text-xs font-bold px-3 py-1 rounded-full">Yangi</span>
                @endif
                @if($product->sale_price)
                    <span class="inline-block bg-red-100 text-red-600 text-xs font-bold px-3 py-1 rounded-full">
                        -{{ round((($product->price - $product->sale_price) / $product->price) * 100) }}% chegirma
                    </span>
                @endif
                @if($product->category)
            ...
            <div class="flex items-center gap-3">
                @if($product->sale_price)
                    <span class="text-3xl font-black text-gray-900">{{ number_format($product->sale_price, 0, '.', ' ') }} <span class="text-lg font-semibold text-gray-500">so'm</span></span>
                    <span class="text-lg text-gray-400 line-through">{{ number_format($product->price, 0, '.', ' ') }} so'm</span>
                @else
                    <span class="text-3xl font-black text-gray-900">{{ number_format($product->price, 0, '.', ' ') }} <span class="text-lg font-semibold text-gray-500">so'm</span></span>
                @endif
            </div>
            ...
            <div class="border-t border-gray-100 pt-5">
                ...
                <div class="mb-5">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @forelse($product->sizes as $size)
                            ...
                            @endif
                        @empty
                            <p class="text-sm text-gray-500">O'lchamlar mavjud emas.</p>
                        @endforelse
                    </div>

                    <p x-show="!selectedSize" class="text-xs text-red-500 mt-2">O'lcham tanlanmagan</p>
                </div>
                ...
                <form action="{{ route('cart.add') }}" method="POST">
                    ...
                    <form action="{{ route('account.wishlist.toggle') }}" method="POST" class="mt-2">
                        ...
                        <button type="submit"
                            ...
                            {{ $inWishlist ? 'Istaklar ro\'yxatidan o\'chirish' : 'Istaklar ro\'yxatiga qo\'shish' }}
                        </button>
                    </form>
                </form>
                @else
                ...
                <div class="mt-5 bg-gray-50 rounded-2xl p-4 space-y-2.5">
                    <div class="flex items-center gap-3 text-sm">
                        ...
                        <span class="text-gray-700">1-3 ish kuni ichida yetkazib beriladi</span>
                    </div>
                    <div class="flex items-center gap-3 text-sm">
                        ...
                        <span class="text-gray-700">100% original mahsulot kafolati</span>
                    </div>
                    <div class="flex items-center gap-3 text-sm">
                        ...
                        <span class="text-gray-700">30 kun ichida qaytarish imkoni</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="mt-12 pt-8 border-t border-gray-100">
        <h2 class="text-xl font-bold text-gray-900 mb-4">Mahsulot haqida</h2>
    ...
    @if($related->count())
    <div class="mt-16">
        <h2 class="text-xl font-bold text-gray-900 mb-6">O'xshash mahsulotlar</h2>
