    ...
    <form action="{{ route('checkout.store') }}" method="POST">
        ...
        <div class="grid lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                {{-- Payment Method --}}
                <div class="bg-white border border-gray-100 rounded-2xl p-6" x-data="{ selected: '{{ old('payment_method', 'cash') }}' }">
                    <h2 class="text-base font-bold text-gray-900 mb-5">{{ __('app.checkout_payment') }}</h2>

                    @error('payment_method')
                        <p class="mb-3 text-xs text-red-500 bg-red-50 border border-red-200 rounded-xl px-4 py-2">{{ $message }}</p>
                    @enderror
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        {{-- Cash --}}
                        <button type="button" @click="selected = 'cash'"
                            :class="selected === 'cash' ? 'border-gray-900 bg-gray-900 text-white shadow-sm' : 'border-gray-200 bg-white text-gray-700 hover:border-gray-400'"
                            class="flex items-center gap-3 px-4 py-3.5 border-2 rounded-xl transition-all duration-200 text-left">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/>
                            </svg>
                            <div>
                                <p class="text-sm font-semibold leading-tight">{{ __('app.payment_cash') }}</p>
                                <p class="text-xs opacity-70 mt-0.5 leading-tight">{{ __('app.payment_cash_desc') }}</p>
                            </div>
                        </button>

                        {{-- Click --}}
                        <button type="button" @click="selected = 'click'"
                            :class="selected === 'click' ? 'border-blue-600 bg-blue-600 text-white shadow-sm' : 'border-gray-200 bg-white text-gray-700 hover:border-blue-400'"
                            class="flex items-center gap-3 px-4 py-3.5 border-2 rounded-xl transition-all duration-200 text-left">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                            </svg>
                            <div>
                                <p class="text-sm font-semibold leading-tight">Click</p>
                                <p class="text-xs opacity-70 mt-0.5 leading-tight">{{ __('app.payment_online_desc') }}</p>
                            </div>
                        </button>

                        {{-- Payme --}}
                        <button type="button" @click="selected = 'payme'"
                            :class="selected === 'payme' ? 'border-teal-600 bg-teal-600 text-white shadow-sm' : 'border-gray-200 bg-white text-gray-700 hover:border-teal-400'"
                            class="flex items-center gap-3 px-4 py-3.5 border-2 rounded-xl transition-all duration-200 text-left">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                            </svg>
                            <div>
                                <p class="text-sm font-semibold leading-tight">Payme</p>
                                <p class="text-xs opacity-70 mt-0.5 leading-tight">{{ __('app.payment_online_desc') }}</p>
                            </div>
                        </button>

                        {{-- Uzum --}}
                        <button type="button" @click="selected = 'uzum'"
                            :class="selected === 'uzum' ? 'border-purple-600 bg-purple-600 text-white shadow-sm' : 'border-gray-200 bg-white text-gray-700 hover:border-purple-400'"
                            class="flex items-center gap-3 px-4 py-3.5 border-2 rounded-xl transition-all duration-200 text-left">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                            </svg>
                            <div>
                                <p class="text-sm font-semibold leading-tight">Uzum Bank</p>
                                <p class="text-xs opacity-70 mt-0.5 leading-tight">{{ __('app.payment_online_desc') }}</p>
                            </div>
                        </button>
                    ...
                    {{-- Cash note --}}
                    <div x-show="selected === 'cash'" x-transition class="mt-4 flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                    ...
                    {{-- Online note --}}
                    <div x-show="selected !== 'cash'" x-transition class="mt-4 flex items-start gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
